...
## 2026-09-08 追加比較: 150 DPIまで拡張

### CP-013: 原本 / 200 / 180 / 150 DPIの目視比較

- Status: Complete（150 DPI比較・自動検証・目視確認完了。製品導入なし）
- Objective: 利用者が150 DPIも同じ配置・倍率で比較できる。
- Dependencies: CP-012の実験script、実資料2冊、既存比較フォルダー、現行photo検証。
- Files or components: `pdf-shrink/experiments/compare_photo_dpi.py`、新規150追加比較parent、当計画。
- Actions: 実験対象を200/180/150 DPIへ拡張、JPEG品質80固定。原本から6候補を独立生成。HTMLへ4番目の切替・並列表示・容量を追加。旧比較一式も保護して前後hashを照合する。
- Completion criteria: 150 DPIの2冊全17ページを目視し、原本/200/180との細部比較を用意。全6候補の検証結果・容量・時間・変更画素寸法を記録する。
- Validation: 既存photo自動検証を緩めず実行、ページ形状/文字/paths/配置一致、200 DPI再現性、原本/既存出力hash不変、HTML操作、関連suite/compile/help/diff。
- Failure conditions: 150 DPIの不合格を合格扱い、原本や既存比較の上書き、製品DPI設定の変更。
- Recovery: 不合格候補も比較用と明示して記録し、製品採用しない。原本と既存比較は保持する。

### CP-013 結果と検証

保存先: `C:\Users\tn\Downloads\KaruFile_写真DPI比較_150追加_20260908`。
`比較.html`に原本/200/180/150の4択、キー1/2/3/4、4枚並列、候補別検査結果、対200/180追加削減を表示。
`site`と`sampling`にそれぞれ`200dpi.pdf`、`180dpi.pdf`、`150dpi.pdf`を生成。
全17ページと48写真領域を4種類で描画した計260 PNG、`comparison.json`、`comparison.md`を保存した。

| 資料 | 200 DPI bytes | 180 DPI bytes | 150 DPI bytes | 180→150追加削減 |
|---|---:|---:|---:|---:|
| 現地写真 | 2,674,925 | 2,204,643 | 1,588,343 | 616,300 bytes / 27.9546% |
| 採取写真 | 1,971,021 | 1,643,718 | 1,228,846 | 414,872 bytes / 25.2399% |
| 合計 | 4,645,946 | 3,848,361 | 2,817,189 | 1,031,172 bytes / 26.7951% |

- 6候補全て現行photo検査と容量条件に合格。qpdf exit 0、全ページ形状・抽出文字・描画パス・画像配置一致。検査閾値・予算上限は無変更。
- 150 DPIで現地28/採取20画像を変更。生成/検査時間は現地2.29/9.90秒、採取1.91/8.04秒。単発観測。
- 独立agentが6PDF実サイズ/hash、qpdf、形状/文字/paths/配置を再検証。279保護ファイルの現在SHA-256もbefore/afterと一致。前回比較一式を含め変更なし。
- 前回と今回の原本/200/180 PNG全195枚がSHA-256完全一致。200/180の画像原寸digest・寸法も前回と一致し、既存目視結果を再利用できる。製品src17ファイルのhashも前回と一致。
- 150 DPI全17ページを目視し、写真欠落・位置ずれ・注記欄の崩れ・目立つ色変化なし。現地p5写真14、p8写真22/23、採取p1写真1を原本/180/150の300 DPI描画で比較。150は黒板の淡い筆画・小字、遠景標識、窓枠/ひび割れの細い枝分かれ、壁や床の細かな質感が180より粗くなる。主対象は全体表示で把握できるが、全細字の可読性/OCRは未評価。
- HTMLをChromeで目視し、キー4、資料/最終ページ/写真範囲、4枚同サイズ、スクロール保持を確認。50%並列時に原本だけcaptionが1行となり画像上端がずれる問題を発見。caption高64 pxで統一し、50/75/100/200/250%で画像幅・高さ・上端が一致、キー1〜4でスクロール400 px保持を再確認。Console error/warnなし。
- 上記修正はHTML表示だけを再生成。PDFと生成証跡は変更せず、`presentation-update.json`に生成時script hashと表示修正後script/HTML hashを区別して保存。更新用一時コマンドの最初の試行はWindows区切りのJSON key不一致で書込み前に停止し、区切り正規化後に成功。
- 関連photo suite **32 passed**、実験script `compileall`/`--help`成功。150/180/200 recipe、未対応DPI拒否、製品150 DPI拒否/200固定の小検査成功。最終`git diff --check`成功。
- 製品runtime変更なし。全component suiteは今回再実行せず、CP-009のEdge確認待ちも別件として維持。担当agentはcommit/pushしていない。

判断: 150 DPIにはこの2冊で180 DPIから合計約1.03 MBの追加削減効果がある。写真の細部の劣化も拡大時に見えるため、自動採用せず比較資料として提供する。通常photo設定は200 DPIのまま。
...
- Status: Complete（比較資料作成・検証完了。製品への180 DPI導入は未実施）
...
保存先: `C:\Users\tn\Downloads\KaruFile_写真DPI比較_20260908`。
`比較.html`で原本・200・180を同じ倍率/位置で切替、または3枚並列表示できる。
全17ページを144 DPI、48写真領域を300 DPIで3種類描画し、合計195 PNGを保存した。
完成PDFは`site/200dpi.pdf`、`site/180dpi.pdf`、`sampling/200dpi.pdf`、`sampling/180dpi.pdf`。
`comparison.json`にrecipe、実行時コードhash、tool version/hash、画像寸法/配置、容量、時間、検査結果を記録した。

| 資料 | 原本bytes | 200 DPI bytes | 180 DPI bytes | 対200 DPI追加削減 |
|---|---:|---:|---:|---:|
| 現地写真 | 3,556,324 | 2,674,925 | 2,204,643 | 470,282 bytes / 17.5811% |
| 採取写真 | 2,539,782 | 1,971,021 | 1,643,718 | 327,303 bytes / 16.6058% |
| 合計 | 6,096,106 | 4,645,946 | 3,848,361 | 797,585 bytes / 17.1673% |

- 4候補とも現行`validate(enhanced=True, detail=True)`合格。qpdf exit 0、原本との全ページ形状・抽出文字・描画パス・画像配置一致。
- 各DPIで現地28画像・採取20画像を変更。2種類の候補を合計すると96回の画像変更。写真の表示寸法を維持し、原本から独立生成した。
- 再生成200 DPIは既存200 DPIと同サイズで、画像の原寸復号画素と全ページ300 DPI RGBも完全一致。保存時PDF IDによるバイトhash差は品質差として扱わない。
- 原本5冊と保護対象の既存出力・証跡、計73ファイルは前後SHA-256不変。独立agentが現在hashも再計算して一致。
- 生成時間: 現地200/180は2.76/2.78秒、採取200/180は1.95/1.81秒。検査/照合は11.70/9.82秒、10.47/8.53秒。200側だけ既存出力との追加照合を含む単発観測であり、速度の優劣を結論にしない。
- 目視: 4完成候補の全34ページ描画を確認。写真の欠落・位置ずれ・文字欄の崩れ・目立つ色変化なし。現地p5黒板・p8遠景標識/窓枠、採取p1黒板を原本も含む300 DPI描画で確認。全体表示では差が小さく、拡大時には180の輪郭・細かな質感が200より少し軟化する。原本でも読みにくい細字があり、全細字可読性/OCR精度は未検証。
- 描画は環境のPyMuPDF 1.28.2を使用（Poppler未検出）。Chromeで比較HTMLを開き、表示を目視、資料/ページ/写真範囲切替、1/2/3キー、3枚同サイズ、スクロール位置300 px保持を確認。実測共通表示幅793.75 CSS px、採取p1写真描画1269×952 px。
- ブラウザーconnectorによる画像/JSONのファイル保存はworkspace root判定で拒否されたため、スクリーンショットはtool返却画像で目視した。保存済み画像証拠とは記載しない。HTML表示・操作は成功。
- 関連suite: `uv run --project pdf-shrink python -m pytest -q pdf-shrink/tests/test_photo_transform.py pdf-shrink/tests/test_photo_worker.py` → **32 passed**。
- 実験scriptの`compileall`、`--help`、`git diff --check`成功。製品runtime未変更のため全component suiteは今回再実行していない。
- 実行前レビューで実効DPI関数の引数を現行APIに修正し、画像切替前から幅/高さを確保してスクロール保持を実装した。実行時script/製品コードhashは独立監査時の現在値とも一致。
- 作業中に別操作でHEADが`cb7df4f05442280a2171fbe6f900e235ff4e8199`から`d9ef8d1879a4a0af0559335f10623a481a5c3128`へ進んだ（19:33:27のcommit）。担当agentはcommit/pushを行っていない。履歴を変更せず検証結果を追記した。

判断: この2冊では180 DPIに追加削減効果がある。閲覧時の許容度を比較できる状態で完了し、通常200 DPI設定は維持する。CP-009のEdge確認待ちは本比較とは別件として残す。
