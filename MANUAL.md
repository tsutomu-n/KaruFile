...
| PDF | 文字PDFと画像だけのスキャンを自動圧縮。スキャンは300 DPI・グレー・JPEG品質92 | 同じPDF処理 |
...
ファイルを直接指定すると、その1冊だけを処理します。周囲のPDF・画像・Excel・動画は探索しません。
出力は `C:\作業\スキャン_軽量化\files\スキャン.pdf`、レポートは
`C:\作業\スキャン_軽量化\report.csv`、状態は同じ親の `.pdf-shrink` です。
`-o`を指定した場合は、そのフォルダーへ元と同じファイル名で保存し、レポートと状態はその親に置きます。
`--dry-run`は判定だけ、再実行は原本・設定・出力の一致を確認して処理済み結果を再利用します。

自動対象は、文字データ（見えないOCR文字も含む）がなく、画像と切り抜き指定で構成されたPDFです。
空白ページは混在できます。写真だけのPDFもこの条件に入り、スキャンか写真かの意味は判別しません。
300 DPIのページ画像へ変換し、グレースケール・JPEG品質92で保存します。色は失われ、
低解像度画像も300 DPIへ再標本化されますが、元より細部が増えるわけではありません。
文字PDFは画像化せず従来の処理を続け、文字・画像混在のPDFも勝手に文字を消しません。

元の寸法・回転・切り抜き範囲・リンク・しおり・文書メタデータを検証し、
原本から独立した可逆候補とも比較して、検証済みで原本より小さい候補だけを採用します。
原本は変更しません。変換後の細かな文字・図は共有前に見比べてください。
圧縮を避けたいPDFには `--pdf-preserve-pattern "スキャン.pdf"` を指定します。
暗号化・署名・フォーム・添付・レイヤー・注釈、上限超過などの保護は引き続きあります。

### PDFの保護と文章向けの指定

判定・候補生成・検証・採用のつながりは[PDF処理の流れと判断基準](docs/PDF_PROCESSING.md)を参照してください。

無指定では文字だけのPDFと、文字データがなく画像だけのスキャンPDFを自動処理します。
スキャン内の切り抜き指定も表示どおりに反映します。文字・画像混在や描画線入りPDFは従来どおり保護します。
...
自動スキャン条件に該当しない未許可の図・画像PDFは可逆圧縮もせず原本保護します。文章向けの詳しい分類・検証は
[技術リファレンス](docs/REFERENCE.md#圧縮対象の判定)を参照してください。
PDF個別CLIの`--safe`では文字候補のグレー化とスキャンの自動画像化を無効にします。写真・文章スキャン・字体統一指定とは併用できません。
