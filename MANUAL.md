...
`--pdf-preview`を付けると、明示選択した写真PDFの原本と実際の完成出力を、ローカルHTMLで
...
`--pdf-preview-dpi`は150〜300の整数を最大5種類まで指定できます。各比較候補は原本から独立に
...
PDFの写真選択パターンや`--pdf-photo-dpi`を変えた場合も再評価します。この版では処理schemaが
...
| コード | 意味 |
|---:|---|
| `0` | 対象となったPDF・画像・動画と、指定したPDF比較のエラーが0件 |
...
拡大もしません。写真用DPIの指定は、写真選択していないPDFの300 DPI目標を変更しません。
